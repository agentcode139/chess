...
public class WebSocketHandler implements WsConnectHandler, WsMessageHandler, WsCloseHandler {
    ...
    public void handleMessage(@NotNull WsMessageContext ctx) {
        try {
            UserGameCommand userGameCommand = new Gson().fromJson(ctx.message(),
                    UserGameCommand.class);
            switch (userGameCommand.getCommandType()) {
                case CONNECT -> connect(userGameCommand, ctx.session);
                case MAKE_MOVE -> makeMove(new Gson().fromJson(ctx.message(), MakeMoveCommand.class),
                        ctx.session);
    ...
    private void connect(UserGameCommand userGameCommand, Session session) throws IOException {
        try {
            ...
            gameClients.computeIfAbsent(userGameCommand.getGameID(),
            ...
            connections.broadcastExclusiveInclude(session,
                    gameClients.get(userGameCommand.getGameID()), notification);
        } catch (Exception e) {
    ...
    private void makeMove(MakeMoveCommand makeMoveCommand, Session session) throws IOException {
        try {
            ...
            connections.broadcastExclusiveInclude(null,
                    gameClients.get(makeMoveCommand.getGameID()), loadGame);

            NoticationMessage notificationMove = new NoticationMessage("move " + move + " was made");
            connections.broadcastExclusiveInclude(session,
                    gameClients.get(makeMoveCommand.getGameID()), notificationMove);
    ...
    private void leave(UserGameCommand userGameCommand, Session session) throws IOException {
        try {
            ...
            Set<Session> clients = gameClients.getOrDefault(userGameCommand.getGameID(),
                    ConcurrentHashMap.newKeySet());
            clients.remove(session);
            String playerColor = (Objects.equals(gameData.whiteUsername(), username)) ? "WHITE" : "BLACK";
            service.leaveGame(userGameCommand.getAuthToken(),
                    new JoinGameRequest(playerColor, userGameCommand.getGameID()));
